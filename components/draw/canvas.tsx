...
export function Canvas() {
  ...
  let selected = useSelector((state: any) => state.draw.selected);

  const refCanvas: any = useRef();
  const refLayer: any = useRef({ current: [] });
  let [move, setMove]: any = useState({
    x: -1,
    y: -1,
    ox: 0,
    oy: 0,
  ...
  const onMouseDown = (event: any, index: any) => {
    event.preventDefault();
